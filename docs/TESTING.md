...
The workflow on `ci/github-actions-checks` includes the existing API unit and HTTP test commands and has been pushed, but has not run on GitHub because its PR is not open yet. Add later test classes to CI only after their equivalent local commands pass. Database integration tests need an isolated test database, committed migrations, synthetic fixtures, and cleanup. See [GIT_WORKFLOW.md](GIT_WORKFLOW.md) for the planned checks.
