...
The original project brief named Jest and ESLint. The user later chose **Vitest**, which is configured in the API scaffold. ESLint remains the intended lint direction, while the generated API currently uses Oxlint; resolve that tooling difference and verify local lint before adding it as a CI gate. Prettier exists in the API scaffold, but a non-writing format check is not configured yet. See [DECISIONS.md](DECISIONS.md) and [DEVELOPMENT.md](DEVELOPMENT.md).
