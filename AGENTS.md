...
This default also covers creating non-Markdown files; deleting or moving files; configuring Clerk, Expo, or Docker; changing environment variables or databases; installing dependencies; running migrations; changing CI configuration; and deploying. The user normally performs these actions after the agent explains them. The agent provides code and test guidance, reviews the user's work and deployment setup, and helps debug failures.
